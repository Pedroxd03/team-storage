...
# Obtener todas las columnas que contienen 'C_rate'
...
for c_rate_col in c_rate_columns:
    for intervalo in data['Interval Number'].unique():
        ...
        if not interval_data.empty:
            ...
            plt.plot(capacidad_mah, interval_data[c_rate_col], label=f'{c_rate_col}', color=colores_c_rate[c_rate_col])
...
handles = [plt.Line2D([0], [0], marker='o', color='w', label=f'{c_rate_col}', markersize=10, markerfacecolor=color) for c_rate_col, color in colores_c_rate.items()]
